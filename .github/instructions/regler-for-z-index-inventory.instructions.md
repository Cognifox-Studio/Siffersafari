...
4. **Pose-specifik utrustning (Backward Compatible):**
   Ny utrustning i `equippedItems` sparas med karaktärens pose-namn som prefix, även för grundposen (`idle_item_safari_hat`, `answerWrong_item_hat_pirate`). Garderoben ska fortfarande läsa äldre oprefixade idle-nycklar som fallback så befintliga profiler på enheterna inte tappar sina val.
